"""Sequential process-isolated ablation benchmark."""
...
with open("scratch/ablation_results_det.json", "w") as f:
    json.dump({"seeds": SEEDS, "results": {v: [s for s in scores] for v, scores in results.items()},
               "stats": {v: stats(scores) for v, scores in results.items()}}, f, indent=2)
